...
namespace vcma{
	...
	template<typename DataType> static size_t ContainerRealloc(HyperContainerContext* hypercontainer_context, DataType*& user_pointer, SIZE_T newSize) {
	...
	template<typename DataType> static clock_t vcrealloc(PVOID hypercontainer_address, DataType*& user_pointer, SIZE_T elements_new_num) {
		HyperContainerContext* hyperContainerContext = vcmalloc_hcm(hypercontainer_address);
		if (hyperContainerContext)
			return ContainerRealloc(hyperContainerContext, user_pointer, elements_new_num);
		else
			throw std::string("Container not found");
	}
